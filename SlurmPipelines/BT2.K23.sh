...
echo "#!/bin/bash
#SBATCH -p normal
#SBATCH --job-name=BT2${FILE/fastq.gz/}
...
mkdir STIS
mkdir Picard
bowtie2 --no-unal --dovetail -k 1 -D 20 -R 3 -N 0 -L 23 -i S,1,0.50 -X2000 -p 40 -x $GENOME -1 ./../rawFastqs/$FILE -2 ./../rawFastqs/${FILE/R1.trim/R2.trim} | samtools view -@ 30 -S -h -b - | samtools sort -@ 30 -o $BAM 
samtools index $BAM
...
sbatch BT2.Align.$OUTNAME
mv BT2.Align.$OUTNAME batchScripts/.
