...
def generatePreProcessScripts(folder,genomeDir,p,q,m,gtfFile,uniqueMap):
	'''
	...
	for i in FASTQ_Files:
		...
		with open(pbsFile,'w') as o:
			...
			write.writerow(('#$','-j', 'y'))
			write.writerow(('#$','-R','y'))
			write.writerow(('#$','-N','RNA_'+i+'_Processing'))
			...
			write.writerow(('module','load','HTSeq/0.6.1/python.2.7.8'))
			...
			write.writerow(('samtools','flagstat',i+'_NoDups.bam','>',i+'_FS.txt'))
			#Cleaning Directory
			...
			write.writerow(('mv',i+'*_FS.txt','samtoolsStats/.'))

			write.writerow(("mkdir","../CountTables"))
			write.writerow(('htseq-count', '-f', 'bam', i+'_NoDups.bam', gtfFile, '>','../CountTables/'+i+'_Counts.txt'))
...
if __name__ == '__main__':
	...
	parser.add_argument('-gtf','--gtfFile')
	...
	gtf = arg.gtfFile
	u = arg.uniqueMap
	generatePreProcessScripts(folder,genomeDir,p,q,m,gtf,u)
